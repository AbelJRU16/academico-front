                ...
                <div class="card-body">
                    <form v-on:submit="guardar($event)">
                        <div class="d-grid col-4 mx-auto mb-3">
                            <img v-if="form.foto" class="foto-form img-thumbnail" :src="form.foto" id="fotoimg" alt="">
                            <img v-else class="foto-form img-thumbnail" src="https://cdn4.iconfinder.com/data/icons/small-n-flat/24/user-256.png" id="fotoimg" alt="">                  
                        </div>
    ...
    export default{
        ...
        methods: { 
            ...
            previsualizarFoto(event){
                ...
                reader.onload = (event) => {
                    let mitFoto = document.getElementById("fotoimg");
                    mitFoto.src = reader.result;
                    this.form.foto = mitFoto.src;
                }
            },
            validateForm(){
                if(this.form.nombre.trim() == ''){
                    ...
                    return false;
                }else if(this.form.apellido.trim() == ''){
            ...
            async getData(id){
                this.cargando = true;
                try{
                    let {data} = await axios.get(`http://academico.test/api/v1/estudiantes/${id}`);
                    this.form = data.data;
                }catch(error){
                    console.error(error);
                }finally{
